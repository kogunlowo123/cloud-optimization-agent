...
def configure_logging(level: str = "INFO", *, json_output: bool = True) -> None:
    """Configure the ``cloudopt`` logger tree.
    ...
    """
    logger = logging.getLogger("cloudopt")
...
def get_logger(name: str) -> logging.Logger:
    """Return a logger namespaced under ``cloudopt``."""
    return logging.getLogger(name if name.startswith("cloudopt") else f"cloudopt.{name}")
